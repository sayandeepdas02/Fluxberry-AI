...
export default function TalentSearchPage() {
    ...
    const { data: fitResponse, isLoading: fitLoading } = useQuery({
        queryKey: ['prospect-fit', selectedProspectId],
        queryFn: () => prospectsApi.getFitAnalysis(selectedProspectId!),
        enabled: !!selectedProspectId,
    })
    const fitInsight = fitResponse?.data
                    ...
                    {detailLoading ? (
                        ...
                        <>
                            ...
                            <div className="flex-1 overflow-y-auto p-6 space-y-5">
                                ...
                                ) : fitInsight && fitInsight.score > 0 ? (
                                    <div className="space-y-3">
                                        <h4 className="text-xs font-semibold uppercase tracking-wider text-muted-foreground flex items-center gap-1.5">
                                            <Sparkles className="w-3.5 h-3.5 text-accent" /> Semantic Fit Analysis
                                        </h4>
                                        ...
                                        <div className="flex items-center justify-between p-3 bg-card/60 border border-line rounded-lg">
                                            <div>
                                                ...
                                                <p className="text-xl font-bold">{fitInsight.score}%</p>
                                            </div>
                                            <div className={`px-2.5 py-1 rounded-full text-[11px] font-semibold ${
                                                fitInsight.score >= 75 ? 'bg-emerald-500/10 text-emerald-400'
                                                : fitInsight.score >= 50 ? 'bg-amber-500/10 text-amber-400'
                                                : 'bg-red-500/10 text-red-400'
                                            }`}>
                                                {fitInsight.score >= 75 ? 'Strong Fit' : fitInsight.score >= 50 ? 'Moderate Fit' : 'Low Fit'}
                                            </div>
                                        </div>

                                        {fitInsight.insights && fitInsight.insights.length > 0 && (
                                            <div>
                                                <p className="text-xs font-medium text-emerald-400 flex items-center gap-1 mb-1.5">
                                                    <TrendingUp className="w-3.5 h-3.5" /> AI Insights
                                                </p>
                                                {fitInsight.insights.map((msg: string, i: number) => (
                                                    <p key={i} className="text-xs text-text-secondary flex items-start gap-1.5 mt-1">
                                                        <span className="w-1 h-1 rounded-full bg-emerald-400 mt-1.5 shrink-0" />{msg}
                                                    </p>
                                                ))}
                                            </div>
                                        )}
                                        <p className="text-[10px] text-muted-foreground/40 text-center italic">ATS Scoring Engine · Compared against highest matching job</p>
                                    </div>
                                ) : (
                                    <div className="p-4 bg-muted/30 rounded-lg text-center text-xs text-muted-foreground">
                                        No matching jobs available to generate semantic fit score.
                                    </div>
                                )}
